...
import org.compiere.process.MemberUpgradeInvoiceGen;
	...
	implements FormPanel, ActionListener, ListSelectionListener
{
	...
	DefaultTableModel 	selTableModel = new DefaultTableModel(
			new String[]{"",
				...
				Msg.translate(Env.getCtx(), "Created By"),
				Msg.translate(Env.getCtx(), "Created Date")
	...
	private void jbInit () throws Exception
	{
		...
		fHistory.setPreferredSize(new Dimension (width,60));
	...
	//	answers.setOpaque(false);
		...
		selTable.getColumnModel().getColumn(3).setPreferredWidth(50);
		
		centerPanel.add (lHistory, new GridBagConstraints (0, ++row, 1, 1, 0.0, 0.0, 
			GridBagConstraints.NORTHEAST, GridBagConstraints.NONE, 
			new Insets (5, 10, 5, 5), 0, 0));
		centerPanel.add (fHistory, new GridBagConstraints (1, row, 3, 1, 0.5, 0.0, 
	...
	public int loadActivities()
	{
		...
		String sql = "SELECT a.* , ad_user.name  FROM AD_WF_Activity a , ad_user "
			+ "WHERE a.createdby = ad_user.ad_user_id AND a.Processed='N' AND a.WFState='OS' AND ("
			//	Owner of Activity
			+ " a.AD_User_ID=?"	//	#1
			//	Invoker (if no invoker = all)
			+ " OR EXISTS (SELECT * FROM AD_WF_Responsible r WHERE a.AD_WF_Responsible_ID=r.AD_WF_Responsible_ID"
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				rowData[5] = activity.get_Value("created");
	...
	{	
		...
		if (e.getSource() == bZoom){
			...
			//Topra modification
			if(ADialog.ask(m_WindowNo, null,"Confirm workflow activity?")){
				//These  threee lines was sended to a methord called 'processWorkFlow' 
				//bOK.setEnabled(false);
				//cmd_OK();
				//bOK.setEnabled(true);
				if(m_activity.get_Value("ad_workflow_id")!=null){
					//Student Registration process Process
					String ad_workflow_id = m_activity.get_Value("ad_workflow_id").toString();
					
					if(ad_workflow_id.equals("100000483"))
					{	
						if(new StdRegWorkFlowValidation(m_activity).validateStudentRegWorkFlow())
						{
							String ad_wf_node_id = m_activity.get_Value("AD_WF_Node_ID").toString();
							if(ad_wf_node_id.equals("100000487")){
								//Student ID and Invoice Generate
								//to generate invoice
								if(new StdRegInvoiceGen(this).stdRegInvoiceGen()){
									this.generateStudentId(m_activity.getRecord_ID());
									this.processWorkFlow();
								}	
							}else if(ad_wf_node_id.equals("100000514")){
								//Student ID Card Printing validation
								MBPartner mbPartner = MBPartner.get(m_activity.getCtx(), m_activity.getRecord_ID());
								mbPartner.set_ValueOfColumn("is_id_printed", "Y");
								
								if(mbPartner.save()){
									this.processWorkFlow();
								}
							}
							else this.processWorkFlow();
						}
						else //Popup user to correct the aplication status 
							ADialog.error(m_WindowNo, this,"Please change the application status, before proceed the workflow!");
					//Member Registration work flow Process	
					}else if(m_activity.get_Value("ad_workflow_id").toString().equals("100000484")){
						//validate the work flow
						if(new MemRegWorkFlowValidation(m_activity).validateMemberRegWorkFlow()){
							//process the document for next node
							if(m_activity.get_Value("AD_WF_Node_ID").toString().equals("100000493")){
								//Member ID and Invoice Generate
								//to generate invoce
								if(new MemRegInvoiceGen(this).memRegInvoiceGen()){
									this.generateMemberRegId(m_activity.getRecord_ID());
									this.processWorkFlow();
								}
							...
							//Popup user to correct the aplication status 
							ADialog.error(m_WindowNo, this,"Please change the application status, before proceed the workflow!");
						}
						
					//Member upgrade work flow Process
					//Member Upgrade-MAAT To FMAAT Direct (AD_Workflow - AD_Workflow_ID=100000485)	
					}else if(m_activity.get_Value("ad_workflow_id").toString().equals("100000485")){
						
						if(new MemUpgradeWrkFlwValidation(this).validateMemUpgrdValidation()){
							//process the document fro next node
							// (AD_WF_Node - AD_WF_Node_ID=1000025)
							if(m_activity.get_Value("AD_WF_Node_ID").toString().equals("100000497")){
								new MemberUpgradeInvoiceGen(this).generateInvoice();
								this.processWorkFlow();
							}else this.processWorkFlow();
						}
						else{
					...
					//Member upgrade workflow Process	
					//Member Upgrade-MAAT to SAT  (AD_Workflow - AD_Workflow_ID=100000486)	
					}else if(m_activity.get_Value("ad_workflow_id").toString().equals("100000486")){
						
						if(new MemUpgradeWrkFlwValidation(this).validateMemUpgrdValidation()){
							//process the document fro next node
							
							if(m_activity.get_Value("AD_WF_Node_ID").toString().equals("100000501")){
								new MemberUpgradeInvoiceGen(this).generateInvoice();
								this.processWorkFlow();
							}else this.processWorkFlow();
						}
					...
					//Member upgrade workflow Process	
					//Member Upgrade-MAAT to SAT Direct (AD_Workflow - AD_Workflow_ID=100000488)	
					}else if(m_activity.get_Value("ad_workflow_id").toString().equals("100000488")){
						
						if(new MemUpgradeWrkFlwValidation(this).validateMemUpgrdValidation()){
							//process the document fro next node
							if(m_activity.get_Value("AD_WF_Node_ID").toString().equals("100000509")){
								new MemberUpgradeInvoiceGen(this).generateInvoice();
								this.processWorkFlow();
							}else this.processWorkFlow();
						}
					...
					//Member upgrade workflow Process	
					//Member Upgrade-SAT to FMAAT Direct  (AD_Workflow - AD_Workflow_ID=100000489)
					}else if(m_activity.get_Value("ad_workflow_id").toString().equals("100000489")){
							
						if(new MemUpgradeWrkFlwValidation(this).validateMemUpgrdValidation()){
							//process the document fro next node
							if(m_activity.get_Value("AD_WF_Node_ID").toString().equals("100000513")){
								new MemberUpgradeInvoiceGen(this).generateInvoice();
								this.processWorkFlow();
							}else this.processWorkFlow();
						}
						...
						//this should be called to proceed another workflows
						this.processWorkFlow();
					}
				}
			}	
	...
	//to process the work flow should be called to this methord
	...
	//topra customization
	private void generateStudentId(int c_bp_id){
		
		try{
			MBPartner bpartner = MBPartner.get(this.m_activity.getCtx(), c_bp_id);
			
			int ad_sequence_id = 1000300;
			MSequence sequence = new MSequence(this.m_activity.getCtx(), ad_sequence_id, this.m_activity.get_TrxName());
			int checkDigit = CheckDigit.getCheckDigit(""+sequence.getCurrentNext());
			Timestamp ts = new Timestamp(System.currentTimeMillis());
			String student_id = ""+sequence.getCurrentNext()+checkDigit;
			
			bpartner.set_ValueOfColumn("c_bp_group_id", 100000485);
			bpartner.set_ValueOfColumn("registered_date", ts);
			bpartner.set_ValueOfColumn("iscustomer", "Y");
			bpartner.set_ValueOfColumn("c_status", "Y");
			bpartner.set_ValueOfColumn("last_renewed_year", Calendar.getInstance().get(Calendar.YEAR));
			bpartner.setValue(student_id);
			bpartner.set_ValueOfColumn("c_student_id", student_id);
			
			bpartner.save();
			//increment the current next
			sequence.setCurrentNext(sequence.getCurrentNext() + sequence.getIncrementNo());
			sequence.save();
			
			ADialog.error(m_WindowNo, this, "New AAT Student ID : "+student_id);
	...
	//Topra customization
	private void generateMemberRegId(int memberId){
		
		try{
			//validate application status SA - Send For Approval
			MBPartner bpartner = MBPartner.get(this.m_activity.getCtx(), memberId);
			if(bpartner.get_Value("c_mem_app_status").equals("MA")){
				
				int ad_sequence_id = 1000994;
				MSequence sequence = new MSequence(this.m_activity.getCtx(), ad_sequence_id, this.m_activity.get_TrxName());
				int checkDigit = CheckDigit.getCheckDigit(""+sequence.getCurrentNext());
				
				String member_id = ""+sequence.getCurrentNext()+checkDigit;
				
				//set bpartner search key and C_MEMBER_ID
				bpartner.setValue(member_id);
				bpartner.set_ValueOfColumn("c_member_id", member_id);
				...
				//Temp MAAT to MAAT
				 //(C_BP_Group - C_BP_Group_ID=100000493) > (C_BP_Group - C_BP_Group_ID=100000492)
				if(C_BP_Group_ID == 100000493){
					C_BP_Group_ID = 100000492;
					group = MBPGroup.get(this.m_activity.getCtx(), C_BP_Group_ID);
					bpartner.set_ValueOfColumn("maat_effective_date", ts);
					bpartner.set_ValueOfColumn("c_maat_id", member_id);
				//Temp SAT to SAT	
				// (C_BP_Group - C_BP_Group_ID=100000494) >  (C_BP_Group - C_BP_Group_ID=100000491)	
				}else if(C_BP_Group_ID == 100000494){
					C_BP_Group_ID = 100000491;
					group = MBPGroup.get(this.m_activity.getCtx(), C_BP_Group_ID);
					bpartner.set_ValueOfColumn("sat_effective_date", ts);
					bpartner.set_ValueOfColumn("c_sat_id", member_id);
				//Temp FMAAT TO FMAAT	
				//(C_BP_Group - C_BP_Group_ID=100000495) >  (C_BP_Group - C_BP_Group_ID=100000490)	
				}else if(C_BP_Group_ID == 100000495){
					C_BP_Group_ID = 100000490;
					group = MBPGroup.get(this.m_activity.getCtx(), C_BP_Group_ID);
					bpartner.set_ValueOfColumn("fmaat_effective_date", ts);
					bpartner.set_ValueOfColumn("c_fmaat_id", member_id);
				}
				
				bpartner.setBPGroup(group);
				bpartner.set_ValueOfColumn("iscustomer", "Y");
				bpartner.set_ValueOfColumn("c_status", "Y");
				bpartner.set_ValueOfColumn("mem_last_renew_year", Calendar.getInstance().get(Calendar.YEAR));
				...
				ADialog.error(m_WindowNo, this, "New AAT Member ID : "+member_id);
			}
			else{
				ADialog.error(m_WindowNo, this, "Approval Unsuccessfull! Please approve  the application by selecting 'Approved status'.");
			}
	...
	public MWFActivity getMActivity(){
		
		return this.m_activity;
	}
	
	public int getWindoseNo(){
