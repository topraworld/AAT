...
public final class VBPartner extends CDialog implements ActionListener
{
	...
	private JComboBox 		fGreetingBP, fGreetingC , bpGroup;
	//
	...
	private void initBPartner()
	{
		...
		bpGroup = new JComboBox (m_bpGroup);
		createLine (bpGroup, "Category", false);
	...
	/**
	 *	Fill Business Partner Groups
	 * 	@return KeyNamePair Array of Groups
	 */
	private Object[] fillBPGroup()
	{
		String sql = "SELECT C_BP_Group_id , name FROM C_BP_Group WHERE ad_client_id = 1000004 AND IsActive='Y' ORDER BY 2";
		sql = MRole.getDefault().addAccessSQL(sql, MBPGroup.Table_Name, MRole.SQL_NOTQUALIFIED, MRole.SQL_RO);
		return DB.getKeyNamePairs(sql, true);
	}
	...
	private boolean actionSave()
	{
		...
		if (m_partner == null)
		{
			...
			int AD_Client_ID = 0;//Env.getAD_Client_ID(Env.getCtx());
			m_partner = MBPartner.getTemplate(Env.getCtx(), AD_Client_ID);
			m_partner.setAD_Org_ID(AD_Client_ID);//Env.getAD_Org_ID(Env.getCtx() // Elaine 2009/07/03
		...
		KeyNamePair bp = (KeyNamePair)bpGroup.getSelectedItem();
		if (bp != null && bp.getKey() > 0){
			m_partner.setC_BP_Group_ID(bp.getKey());
		}
		else
