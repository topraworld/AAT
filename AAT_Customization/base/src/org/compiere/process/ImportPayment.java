...
public class ImportPayment extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		//create new students and members
		//this.createBpartner();
	...
	//Topra Customization
	//Main purpose is , When uploading new AAT students/members payments, Create the student/member
	
	private enum c_bp_group{
		
		Temp_STUDENT(100000484) , Temp_MAAT(100000493),Temp_FMAAT(100000495) , Temp_SAT(100000494) , BSStudent(100000497),
		OTHR_CUSTM(100000498) , WEEKEND_CLASS(100000499);
	...
	private void validateBpartners(){
		try{
			
			String sqlWhere = "c_doctype_id=1000059 AND i_isimported!='Y'"
					+ " AND c_bpartner_id IS null AND isactive = 'Y'"
					+ " AND ad_client_id = " + this.getAD_Client_ID();
			int [] rows = X_I_Payment.getAllIDs("I_Payment", sqlWhere , get_TrxName());
			
			for(int row : rows){
				X_I_Payment x_I_Payment = new X_I_Payment(m_ctx, row, get_TrxName());
				//validate the process for only "AR Receipt" (C_DocType - C_DocType_ID=1000059)
				if(x_I_Payment.getC_DocType_ID() != 1000059)
					continue;
				this.validatePayment(x_I_Payment);
			}
			
		}catch(Exception ex){
			ADialog.error(0, null, "Errors - Please check data you have imported!!");
			ex.printStackTrace();
		}
	}
	
	@SuppressWarnings("unused")
	private void validatePayment(X_I_Payment x_I_Payment) throws Exception{
		
		int prefix = 0;
		int length = 0;
		
		String docno = x_I_Payment.getDocumentNo().trim();
		prefix = Integer.parseInt(docno.substring(0, 2));
		int number = Integer.parseInt(docno.substring(2, docno.length()));
		length = docno.length() - 2;
		...
		if(prefix == 11){
			this.createNewBp(number, c_bp_group.Temp_STUDENT.value, x_I_Payment);
		//Examination Existing Students
		}else if(prefix == 22){
			//MBPartner.get(m_ctx, Integer.toString(number));
			x_I_Payment.setC_BPartner_ID(MBPartner.get(m_ctx, Integer.toString(number)).get_ID());
		//Exemption	
		}else if(prefix == 33){
			//new Student
			if(length == 5){
		...
		//new Member	
			...
			//Member Renew
			if(length == 5){
				this.createNewBp(number, c_bp_group.Temp_MAAT.value, x_I_Payment);
			}else if(length == 6){
				x_I_Payment.setC_BPartner_ID(MBPartner.get(m_ctx, Integer.toString(number)).get_ID());
			//Student request
			}else if(length == 7){
		...
		//create new student	
			...
			this.createNewBp(number, c_bp_group.WEEKEND_CLASS.value, x_I_Payment);
		//Other payments	
		//create new Other 
		}else if(prefix == 75){
			this.createNewBp(number, c_bp_group.OTHR_CUSTM.value, x_I_Payment);
		//PF Seminer	
		}else if(prefix == 77){
			x_I_Payment.setC_BPartner_ID(MBPartner.get(m_ctx, Integer.toString(number)).get_ID());
		//PF Recomondation letters
		}else if(prefix == 88){
			x_I_Payment.setC_BPartner_ID(MBPartner.get(m_ctx, Integer.toString(number)).get_ID());
		//Business Shoool
		//new bpartner	
		}else if(prefix == 99){
			this.createNewBp(number, c_bp_group.BSStudent.value, x_I_Payment);
		}
		x_I_Payment.save();
	}
	
	private void createNewBp(int value ,int C_BP_Group_ID , X_I_Payment x_I_Payment){

		if(MBPartner.get(m_ctx, Integer.toString(value)) == null){
			MBPartner mbPartner = new MBPartner(m_ctx, 0, get_TrxName());
			mbPartner.setAD_Org_ID(0);
			mbPartner.setName(Integer.toString(value));
			mbPartner.setValue(Integer.toString(value));
			mbPartner.setC_BP_Group_ID(C_BP_Group_ID);
			mbPartner.save();
			x_I_Payment.setC_BPartner_ID(mbPartner.get_ID());
			x_I_Payment.save();
		}
	}
