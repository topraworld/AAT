			...
			{	
				...
				if ( ! New_BPValue.equals(Old_BPValue)) {
					...
					else				//	Update existing BPartner
					{
						...
						if (impBP.get_Value("i_registered_date") != null)
							bp.set_ValueOfColumn("registered_date", impBP.get_Value("i_registered_date"));
						if (impBP.get_Value("i_aplication_entered") != null)
							bp.set_ValueOfColumn("aplication_entered", impBP.get_Value("i_aplication_entered"));
						if (impBP.get_Value("i_birthday") != null)
							bp.set_ValueOfColumn("birthday", impBP.get_Value("i_birthday"));
					...
					/*
					MBPartnerLocation[] i = bp.getLocations(true);
					for(MBPartnerLocation mbPartnerLocation : i){
						//System.out.println(mbPartnerLocation.get_ID());
						bpl = new MBPartnerLocation(getCtx(), mbPartnerLocation.get_ID(), get_TrxName());
						MLocation location = new MLocation(getCtx(), bpl.getC_Location_ID(), get_TrxName());
						location.set_CustomColumn("address3", impBP.get_Value("i_address3"));
					...
					if (impBP.getC_BPartner_Location_ID() != 0)		//	Update Location
					{
								...
								&& impBP.getCity() != null)
						{
							...
							//insert custom locations
							bpl.set_CustomColumn("c_location_district", impBP.get_Value("i_c_location_district").toString().trim());
							bpl.set_CustomColumn("email", impBP.get_Value("i_email"));
